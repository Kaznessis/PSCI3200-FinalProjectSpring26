...
To test the hypothesis, the analysis links historical slave trade exposure to contemporary cancer screening outcomes using spatially matched datasets. The project draws from [Nathan Nunn’s Slave Trade Database](https://nathannunn.arts.ubc.ca/data/), which estimates the number of individuals exported from Africa by ethnic groups during the transatlantic and Indian Ocean slave trades. The key independent variable is slave trade intensity, measured as the number of individuals exported from each ethnic group in Nunn (2008). This metric, demonstrating slave trade extraction by ethnic group, is merged with [Murdock’s Ethnic Homeland Shapefile](https://nathannunn.arts.ubc.ca/data/), which geolocates historical ethnic territories across Africa.
...
\(1\) **Cervical Cancer Screening Rates Spatial Mapping**

![](images/clipboard-2354182760.png)

\(2\) **Slave Trade Intensity Spatial Mapping**

![](images/clipboard-3039526766.png)
...
**\
Cervical Cancer Screening Rates vs. Log Slave Export Intensity**\
![](images/clipboard-767181625.png)\
The slave exports metric was logged as the raw slave export variable is highly skewed and non-linear in its relationship with outcomes. Additionally, please be aware that the following regression has yet to incorporate the control for confounders and country-fixed effects. The full specification will be implemented in future analysis.
