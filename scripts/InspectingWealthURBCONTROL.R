...
############################################################
# 3. PREP DHS CLUSTER-LEVEL WEALTH AND URBANIZATION
############################################################
data <- data %>%
  mutate(wt = perweight / 1000000)

cluster_data <- data %>%
  filter(!is.na(wealthq), !is.na(urban), !is.na(clusterno)) %>%
  group_by(clusterno) %>%
  summarize(
    mean_wealth_cluster = weighted.mean(wealthq, wt, na.rm = TRUE),
    mean_urban_cluster = weighted.mean(urban == 1, wt, na.rm = TRUE),
    n = n(),
    .groups = "drop"
  ) %>%
...
############################################################
# 7. AGGREGATE WEALTH AND URBANIZATION TO ETHNIC LEVEL
############################################################
ethnic_summary <- cluster_joined %>%
  ...
  summarize(
    ...
    mean_urban = weighted.mean(mean_urban_cluster, w = n, na.rm = TRUE),
    n_clusters = dplyr::n(),
...
# Bivariate model: extraction only
model_wealth <- lm(
  mean_wealth ~ ln_total_exports,
  data = plot_data
)

summary(model_wealth)

# Controlled model: extraction controlling for urbanization
model_wealth_controls <- lm(
  mean_wealth ~ ln_total_exports + mean_urban,
  data = plot_data
)

summary(model_wealth_controls)

############################################################
# 14. COEFFICIENT TABLES
############################################################
coef(summary(model_wealth))
coef(summary(model_wealth_controls))

############################################################
# 15. READABLE CONTROLLED REGRESSION EQUATION
############################################################
wealth_controls_table <- coef(summary(model_wealth_controls))

wealth_intercept <- wealth_controls_table["(Intercept)", "Estimate"]
wealth_exports_coef <- wealth_controls_table["ln_total_exports", "Estimate"]
wealth_urban_coef <- wealth_controls_table["mean_urban", "Estimate"]

cat("\nControlled wealth regression equation:\n")
cat(
  "mean_wealth = ",
  round(wealth_intercept, 4),
  " + ",
  round(wealth_exports_coef, 4),
  " * ln_total_exports + ",
  round(wealth_urban_coef, 4),
  " * mean_urban\n",
  sep = ""
)
