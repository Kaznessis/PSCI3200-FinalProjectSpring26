...
#gps_file <- "/Users/serafeimkaznessis/Downloads/ZMGE81FL/ZMGE81FL.shp"
...
ggplot(plot_data, aes(x = ln_total_exports, y = mean_outcome)) +
  ...
  theme_minimal() +
  labs(
    title = "Regression Line: Extraction vs Mean Outcome",
    x = "Log Historical Exports",
    y = "Mean Outcome"
  )

# Controlled model: only controls for urban residence
model_controls <- lm(
  mean_outcome ~ ln_total_exports + mean_urban,
  data = plot_data
)
...
############################################################
# 20. SAVE CONTROLLED REGRESSION OUTPUT
############################################################
dir.create(
