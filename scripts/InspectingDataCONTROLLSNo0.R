...
############################################################
# 8. LOAD COUNTRY BOUNDARY AND CLIP ETHNIC POLYGONS
############################################################
...
############################################################
# 11. MERGE ETHNIC-LEVEL OUTCOME BACK TO POLYGONS
############################################################
ethnic_country <- ethnic_country %>%
  left_join(ethnic_summary, by = "NAME")

############################################################
# 12. PREP NUNN EXTRACTION DATA
############################################################
nunn <- nunn %>%
  mutate(
...
############################################################
# 13. MERGE EXTRACTION DATA ONTO ETHNIC POLYGONS
# DROP ZERO-EXTRACTION ETHNIC HOMELANDS
############################################################
ethnic_country <- ethnic_country %>%
  select(-matches("^total_exports|^ln_total_exports")) %>%
  left_join(nunn_ethnic, by = c("NAME" = "murdock_name")) %>%
  filter(!is.na(ln_total_exports), ln_total_exports > 0)

cat("Matched ethnic groups with positive extraction data:",
    nrow(ethnic_country), "\n")

############################################################
# 14. MAP 1: OUTCOME BY ETHNIC TERRITORY
############################################################
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = paste0("Ethnic-Level Outcome: ", analysis_var, " (", country_name, ", Positive Extraction Only)"),
    fill = "Outcome Rate"
  )

############################################################
# 15. MAP 2: EXTRACTION INTENSITY BY ETHNIC TERRITORY
############################################################
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = paste0("Historical Extraction Intensity by Ethnicity (", country_name, ", Positive Extraction Only)"),
    fill = "Log Exports"
  )

############################################################
# 16. SCATTERPLOT: EXTRACTION VS OUTCOME
############################################################
plot_data <- ethnic_country %>%
  st_drop_geometry() %>%
  filter(!is.na(ln_total_exports), !is.na(mean_outcome), ln_total_exports > 0)
...
ggplot(plot_data, aes(x = ln_total_exports, y = mean_outcome)) +
  ...
  theme_minimal() +
  labs(
    title = paste0("Extraction Intensity vs ", analysis_var, " (", country_name, ", Positive Extraction Only)"),
...
ggplot(plot_data, aes(x = ln_total_exports, y = mean_outcome)) +
  ...
  theme_minimal() +
  labs(
    title = "Regression Line: Extraction vs Mean Outcome (Positive Extraction Only)",
    x = "Log Historical Exports",
    y = "Mean Outcome"
  )
...
############################################################
# 18. OPTIONAL: MAP RAW CLUSTERS FOR SANITY CHECK
############################################################
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = paste0("Cluster-Level ", analysis_var, " (", country_name, ", Positive Extraction Only)"),
    color = "Cluster Rate"
  )
...
output_file <- paste0(
  ...
  country_name, "_",
  analysis_var,
  "_model_controls_summary_positive_only.txt"
)
